...
public class OneToOneRingBufferTest {
    ...
    /**
     * 初始 capacity 必须大于 TRAILER_LENGTH + 16
     */
    @Test
    public void shouldThrowExceptionIfCapacityIsBelowMinCapacity() {
        for (int capacity : new int[]{2, 4, 8}) {
    ...
    public void shouldRejectWriteWhenInsufficientSpace() {
        ...
        final long head = 0L;
        final long tail = head + (CAPACITY - BitUtil.align(length - ALIGNMENT, ALIGNMENT));
    ...
    public void shouldCalculateCapacityForBuffer() {
        assertEquals(ringBuffer.capacity(), CAPACITY);
    }
...
//    }
    ...
    @Test
    public void shouldAllowWritingEmptyMessagesWhenCapacityIsMinimal() {
    ...
    @Test
    public void shouldWriteMessageAfterInsertedPaddingIsConsumedThusMakeEnoughContiguousSpace() {
        ...
        srcBuffer.putLong(0, Long.MAX_VALUE);
        assertTrue(ringBuffer.write(msgType, srcBuffer, 0, BitUtil.SIZE_OF_LONG));

        assertTrue(ringBuffer.write(msgType, srcBuffer, 0, 0)); // an empty message

        srcBuffer.putLong(0, Long.MIN_VALUE);
        assertFalse(ringBuffer.write(msgType, srcBuffer, 0, BitUtil.SIZE_OF_LONG)); // not enough space in the buffer
        ...
        assertFalse(ringBuffer.write(msgType, srcBuffer, 0, BitUtil.SIZE_OF_LONG));

        // consume the empty message and move head
        assertEquals(1, ringBuffer.read((msgTypeId, buffer, index, length) -> assertEquals(0, length)));

        assertTrue(ringBuffer.write(msgType, srcBuffer, 0, BitUtil.SIZE_OF_LONG)); // message fits
